...
public class AutoPriceDAOImpl extends DAOImpl<AutoPrice> implements AutoPriceDAO {

    private final Logger logger = Logger.getLogger(ConcretePriceDAOImpl.class.getName());
    ...
    @Override
    public List<AutoPrice> getAllAutoPrice() {
        logger.log(Level.INFO, START_GET_ALL_AUTO_PRICE);
        List<AutoPrice> list = new ArrayList<>();
    ...
    public Double getAutoPriceByMobilityAndVolumeOfConcrete(Mobility mobility, Double volume, Double distance) {
        ...
        if (distance < TRANSPORT_CONSTANT) {
            ...
            coefficient = autoCapacityDAO.getAutoCapacityByCapacity(existCapacity.get(ZERO)).getDeliveryCoefficientAft50();
        }
